...
int16_t*** get_candidate_freq_bins(float halfBinWidth, int16_t numHarmonics, float* f0Cands, int16_t* f0CandsNumHarmonics, int16_t** f0CandsHarmonicNumBins){
	...
	int16_t*** candidateFreqBins = (int16_t***) malloc(sizeof(int16_t**) * NUM_F0_CANDS);
	int16_t h;
	...
	for (int i = 0 ; i < NUM_F0_CANDS ; i++){
		...
		candidateFreqBins[i] = (int16_t**) malloc(sizeof(int16_t*) * f0CandsNumHarmonics[i]);
		...
		for (h = 1; h < f0CandsNumHarmonics[i] + 1  ; h++){
			...
			freq = f0Cands[i]*h;
				...
				candidateFreqBins[i][h-1] = range_int(lowFreqBin, highFreqBin + 1);
...
void polyphonic_init(Polyphonic* p){
	...
	p -> f0CandsFreqBins = get_candidate_freq_bins(p -> halfBinWidth, p -> numHarmonics, p -> f0Cands, p -> f0CandsNumHarmonics, p -> f0CandsHarmonicNumBins);
...
void polyphonic_deinit(Polyphonic* p){
	...
	for(int i = 0 ; i < NUM_F0_CANDS ; i ++){
		for (int h = 0 ; h < p -> f0CandsNumHarmonics[i] ; h ++){
			free(p ->  f0CandsFreqBins[i][h]);
			p ->  f0CandsFreqBins[i][h] = NULL;
		}
		free(p -> f0CandsFreqBins[i]);
		p -> f0CandsFreqBins[i] = NULL;
